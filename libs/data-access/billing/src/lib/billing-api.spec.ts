import { fetchPlans, fetchSubscription, subscribeToPlan, cancelSubscription, fetchInvoices } from './billing-api';
...
describe('billing-api', () => {
  it('fetchPlans sends GET /api/billing/plans', async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ data: [{ id: 'p1', name: 'Free', tier: 'free', price: 0, interval: 'monthly', features: [] }] }),
    });
  ...
  it('fetchSubscription sends GET /api/billing/subscription', async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ data: { id: 's1', planId: 'p1', status: 'active' } }),
    });
  ...
  it('subscribeToPlan sends POST /api/billing/subscribe', async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ data: { id: 's2', planId: 'p2', status: 'active' } }),
    });
    const result = await subscribeToPlan('p2', 'monthly');
    expect(mockFetch).toHaveBeenCalledWith('/api/billing/subscribe', expect.objectContaining({ method: 'POST' }));
  ...
  it('cancelSubscription sends POST /api/billing/cancel', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith('/api/billing/cancel', expect.objectContaining({ method: 'POST' }));
  ...
  it('fetchInvoices sends GET /api/billing/invoices', async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({ data: [{ id: 'inv1', amount: 29, status: 'paid' }] }),
    });
